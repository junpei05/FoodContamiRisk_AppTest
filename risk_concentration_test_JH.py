...
with col3:
    ...
    # 汚染濃度の平均と標本標準偏差の計算
    mean_concentration = df_bacteria_counts['汚染濃度 [log CFU/g]'].mean()
    std_concentration = df_bacteria_counts['汚染濃度 [log CFU/g]'].std(ddof=1)
    # 平均と標準偏差の表示用データフレームを作成
    stats_df = pd.DataFrame({
        '統計量': ['平均', '標準偏差'],
        '汚染濃度 [log CFU/g]': [mean_concentration, std_concentration]
    })
    # 統計情報を表示
    st.table(stats_df)
...
for bacteria_name, df_bacteria in bacteria_data:
    if not df_bacteria.empty:
        ...
        with col5:
            ...
            # 汚染濃度の平均と標本標準偏差の計算
            mean_conc = df_bacteria_conc['汚染濃度 [log CFU/g]'].mean()
            std_conc = df_bacteria_conc['汚染濃度 [log CFU/g]'].std(ddof=1)
            # 平均と標準偏差の表示用データフレームを作成
            stats_df = pd.DataFrame({
                '統計量': ['平均', '標準偏差'],
                '汚染濃度 [log CFU/g]': [mean_conc, std_conc]
            })
            # 統計情報を表示
            st.table(stats_df)
